...
export class AdminService {
  ...
  async processApplication(processRequest: ApplicationProcessRequest): Promise<ApplicantDTO>{
    const applicantRepo = new ApplicantRepository()
    let applicant = await applicantRepo.findByAadharNumberAndApplicationNumber(processRequest.aadharNumber, processRequest.applicationNumber);
    if (!applicant) {
  ...
  private getFilteredApplicants(applicants: Applicant[]): ApplicantDTO[] {
    ...
    applicants.forEach(applicant => {
      ...
      const filteredApplicant: ApplicantDTO = this.convertApplicationToDTO(applicant, grantApplication);
  ...
  private convertApplicationToDTO(applicant: Applicant, grantApplication: GrantApplication): ApplicantDTO {
